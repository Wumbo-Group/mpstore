...
$ambil = $koneksi->query("SELECT * FROM `order` LEFT JOIN order_detail ON `order`.id_order = order_detail.id_order LEFT JOIN produk ON order_detail.id_produk = produk.id_produk LEFT JOIN kategori ON produk.id_kategori = kategori.id_kategori WHERE kategori.id_kategori_awal = 2 AND `order`.status_pembayaran ='pending' AND `order`.bool_wa = 0");
...
SALAM BERKAH MULIA
CS MP Store - Seminar";
        wa_seminar($no_hp_pembeli, $pesan);
    }
    elseif ($status_transaksi == 'settlement') {
        //kirim wa ke pembeli pesan sukses seminar
        $bool_wa = 1;
$pesan = "Assalamu'alaikum Warohmatullahi Wabarokatuh
...
CS MP Store - Seminar";
        ...
        //kirim wa ke pembeli pesan gagal seminar
        $bool_wa = 1;
$pesan = "Assalamu'alaikum Warohmatullahi Wabarokatuh
