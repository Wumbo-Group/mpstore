...
$ambil = $koneksi->query("SELECT * FROM `order` LEFT JOIN order_detail ON `order`.id_order = order_detail.id_order LEFT JOIN produk ON order_detail.id_produk = produk.id_produk LEFT JOIN kategori ON produk.id_kategori = kategori.id_kategori WHERE kategori.id_kategori_awal = 0 OR kategori.id_kategori_awal = 5 AND `order`.status_pembayaran ='pending' AND `order`.bool_wa = 0");
...
SALAM BERKAH MULIA
CS MP Store";
        wa_mpstore($no_hp_pembeli, $pesan);
    }
    elseif ($status_transaksi == 'settlement') {
        //kirim wa ke pembeli pesan sukses mpstore
        $bool_wa = 1;
$pesan = "Assalamu'alaikum Warohmatullahi Wabarokatuh
...
CS MP Store";
        ...
        //kirim wa ke pembeli pesan gagal mpstore
        $bool_wa = 1;
$pesan = "Assalamu'alaikum Warohmatullahi Wabarokatuh
