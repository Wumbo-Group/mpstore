<?php 
    ...
    if ($data_sort == "4") {
        if ($linkct == "0") {
            $produk_page = $koneksi->query("SELECT COUNT(*) As total_records FROM produk WHERE nama_produk LIKE '%".$dataPencarian."%' AND stok > 0 AND jenis_produk='barang'");
            $total_records = $produk_page->fetch_array();
            $total_records = $total_records['total_records'];
            $total_no_of_pages = ceil($total_records / $total_records_per_page);
            $second_last = $total_no_of_pages - 1;

            $produk = $koneksi->query("SELECT * FROM produk WHERE nama_produk LIKE '%".$dataPencarian."%' AND stok > 0 AND jenis_produk='barang' ORDER BY id_produk ASC LIMIT $offset, $total_records_per_page");
        }
        else {
            $produk_page = $koneksi->query("SELECT COUNT(*) As total_records FROM produk WHERE id_kategori = '$linkct' AND nama_produk LIKE '%".$dataPencarian."%' AND stok > 0 AND jenis_produk='barang'");
            $total_records = $produk_page->fetch_array();
            $total_records = $total_records['total_records'];
            $total_no_of_pages = ceil($total_records / $total_records_per_page);
            $second_last = $total_no_of_pages - 1;

            $produk = $koneksi->query("SELECT * FROM produk WHERE id_kategori = '$linkct' AND nama_produk LIKE '%".$dataPencarian."%' AND stok > 0 AND jenis_produk='barang' ORDER BY id_produk ASC LIMIT $offset, $total_records_per_page");
        }
    }
    if ($data_sort == "5") {
        if ($linkct == "0") {
            $produk_page = $koneksi->query("SELECT COUNT(*) As total_records FROM produk WHERE nama_produk LIKE '%".$dataPencarian."%' AND stok > 0 AND jenis_produk='jasa'");
            $total_records = $produk_page->fetch_array();
            $total_records = $total_records['total_records'];
            $total_no_of_pages = ceil($total_records / $total_records_per_page);
            $second_last = $total_no_of_pages - 1;

            $produk = $koneksi->query("SELECT * FROM produk WHERE nama_produk LIKE '%".$dataPencarian."%' AND stok > 0 AND jenis_produk='jasa' ORDER BY id_produk ASC LIMIT $offset, $total_records_per_page");
        }
        else {
            $produk_page = $koneksi->query("SELECT COUNT(*) As total_records FROM produk WHERE id_kategori = '$linkct' AND nama_produk LIKE '%".$dataPencarian."%' AND stok > 0 AND jenis_produk='jasa'");
            $total_records = $produk_page->fetch_array();
            $total_records = $total_records['total_records'];
            $total_no_of_pages = ceil($total_records / $total_records_per_page);
            $second_last = $total_no_of_pages - 1;

            $produk = $koneksi->query("SELECT * FROM produk WHERE id_kategori = '$linkct' AND nama_produk LIKE '%".$dataPencarian."%' AND stok > 0 AND jenis_produk='jasa' ORDER BY id_produk ASC LIMIT $offset, $total_records_per_page");
        }
    }
    $arrayId = array();
    ...
    <div class='col-sm-12 col-md-6 col-lg-4 p-b-50'>
        <div class='block2'>
            <div class='block1 block2-img wrap-pic-w of-hidden pos-relative block2-labelnew'>
                <img src='images/produk/produk_barang/".$dProduk['foto_produk1']."?nocache=".time()."' class='block1' alt='IMG-PRODUCT'>
