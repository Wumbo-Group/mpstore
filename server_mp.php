...
$ambil = $koneksi->query("SELECT * FROM `order` LEFT JOIN order_detail ON `order`.id_order = order_detail.id_order LEFT JOIN produk ON order_detail.id_produk = produk.id_produk WHERE produk.id_kategori = 1 AND `order`.status_pembayaran ='pending' AND `order`.bool_wa = 0");
while ($pecah = $ambil->fetch_assoc()) {
    $idMem = $pecah['id_member'];
    $id_order = $pecah['id_order'];
    $no_hp_pembeli = $pecah['nohp_pembeli'];
    $email_pembeli = $pecah['email_pembeli'];
...
CS MP Store - Muslimpreneurship";
        ...
        elseif ($status_transaksi == 'settlement') {
            //kirim wa ke siswa pesan sukses Muslimpreneurship
$pesan = "Assalamu'alaikum Warohmatullahi Wabarokatuh
...
CS MP Store - Muslimpreneurship";
            ...
            //kirim wa ke siswa pesan gagal Muslimpreneurship
$pesan = "Assalamu'alaikum Warohmatullahi Wabarokatuh
...
SALAM BERKAH MULIA
CS MP Store - Muslimpreneurship";
        wa_mp($no_hp_pembeli, $pesan);
        // resetPassword($email_pembeli);
        // echo "-akun dibuat-";
    }
    elseif ($status_transaksi == 'settlement') {
        //kirim wa ke pembeli pesan sukses Muslimpreneurship
        $bool_wa = 1;
$pesan = "Assalamu'alaikum Warohmatullahi Wabarokatuh
...
Informasi akun MP Store anda akan segera kami kirimkan melalui email.
...
CS MP Store - Muslimpreneurship";
        ...
        //kirim wa ke pembeli pesan gagal Muslimpreneurship
        $bool_wa = 1;
$pesan = "Assalamu'alaikum Warohmatullahi Wabarokatuh
...
CS MP Store - Muslimpreneurship";
        ...
        // Hapus Akun MP Store
        $koneksi->query("DELETE from user where email_user='$email_pembeli'");
        echo "-akun dihapus-";
