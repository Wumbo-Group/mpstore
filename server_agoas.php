...
$ambil = $koneksi->query("SELECT * FROM `order` LEFT JOIN order_detail ON `order`.id_order = order_detail.id_order LEFT JOIN produk ON order_detail.id_produk = produk.id_produk WHERE produk.id_kategori = 2 AND `order`.status_pembayaran ='pending' AND `order`.bool_wa = 0");
...
SALAM BERKAH MULIA
CS MP Store - AGOAS";
        wa_agoas($no_hp_pembeli, $pesan);
    }
    elseif ($status_transaksi == 'settlement') {
        //kirim wa ke pembeli pesan sukses agoas
        $bool_wa = 1;
$pesan = "Assalamu'alaikum Warohmatullahi Wabarokatuh
...
CS MP Store - AGOAS";
        ...
        //kirim wa ke pembeli pesan gagal agoas
        $bool_wa = 1;
$pesan = "Assalamu'alaikum Warohmatullahi Wabarokatuh
